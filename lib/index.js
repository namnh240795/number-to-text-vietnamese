import { isNumber, addSeperator } from "./utils";
...
const getThousand = (number) => {
  ...
  if (beforeLength === 1) {
    return `${base[before_number]} nghìn${afterText ? ' ' + afterText : ''}`;
  }

  if (beforeLength === 2) {
    return `${getTen(before_number)} nghìn${afterText ? ' ' + afterText : ''}`;
  }
...
const getMillion = (number) => {
  ...
  if (beforeLength === 1) {
    return `${base[before_number]} triệu${afterText ? ' ' + afterText : ''}`;
  }

  if (beforeLength === 2) {
    return `${getTen(before_number)} triệu${afterText ? ' ' + afterText : ''}`;
  }
...
const getBillion = (number) => {
  ...
  const beforeLength = `${before_number}`.length;
  if (beforeLength === 1) {
    return `${base[before_number]} tỷ${afterText ? ' ' + afterText : ''}`;
  }

  if (beforeLength === 2) {
    return `${getTen(before_number)} tỷ${afterText ? ' ' + afterText : ''}`;
  }

  if (beforeLength === 3) {
    return `${getHundred(before_number)} tỷ${afterText ? ' ' + afterText : ''}`;
  }

  if (beforeLength > 3 && beforeLength <= 6) {
    return `${getThousand(before_number)} tỷ${afterText ? ' ' + afterText : ''}`;
  }

  if (beforeLength > 6 && beforeLength <= 9) {
    return `${getMillion(before_number)} tỷ${afterText ? ' ' + afterText : ''}`;
  }
};

export const getText = (number, seperator = '') => {
  // T021: Fix error handling - check type first
  if (!isNumber(number)) {
    throw new Error("Input is not a number");
  }

  // T019: Fix NaN handling
  if (Number.isNaN(number)) {
    throw new Error("Input is not a number");
  }

  // Check for Infinity before integer check
  if (!isFinite(number)) {
    throw new Error("Your number is too big");
  }

  // T020: Fix float handling
